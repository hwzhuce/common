...
namespace Common
{
    public class JsonHelper
    {
        ...
        /// <summary>
        /// 将字符串中的特殊字符进行转义(保留html)
        /// </summary>
        /// <param name="orgData">字符串</param>
        /// <returns></returns>
        public static string ReplaceSpecialCharactersAllowHtml(string orgData)
        {
            orgData = orgData.
                              Replace("\"", "'").
                              Replace("\\", "\\\\");
        ...
        /// <param name="name">json格式数据参数名称</param>
        /// <returns>json格式数据</returns>
        public static string DataTableToJsonDataAlowHtml(DataTable dt)
        {
            StringBuilder jsonStr = new StringBuilder("[");
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    foreach (DataColumn cloumn in dt.Columns)
                    {
                        jsonStr.Append("\"" + cloumn.ColumnName.ToLower() + "\":\"" + ReplaceSpecialCharactersAllowHtml(row[cloumn.ColumnName].ToString()) + "\",");
                    }
                    jsonStr.Remove(jsonStr.Length - 1, 1);
                    jsonStr.Append("},");
                }
        ...
        /// <param name="name">json格式数据参数名称</param>
        /// <returns>json格式数据</returns>
        public static string DataTableToJsonDataFliterValidChar(DataTable dt)
        {
            StringBuilder jsonStr = new StringBuilder("[");
            if (dt.Rows.Count > 0)
            {
        ...
        /// <param name="rowName">行记录参数名称</param>
        /// <returns>json格式</returns>
        public static string CreateJsonParametersFliterValidChar(DataTable dt, bool displayCount, int totalCount, string jsonName, string rowName)
        {
            StringBuilder jsonStr = new StringBuilder();
            if (dt != null)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < dt.Columns.Count; j++)
                    {
                        if (j < dt.Columns.Count - 1)
                        {
                            ...
                            {
                                jsonStr.Append("\"" + dt.Columns[j].ColumnName.ToLower() + "\":" + "\"" +
                                                  ReplaceSpecialCharacters(dt.Rows[i][j].ToString().Replace("\"", "\\\"")) + "\",");
                            }
                            else if (dt.Columns[j].DataType == typeof(DateTime))
                            {
                                jsonStr.Append("\"" + dt.Columns[j].ColumnName.ToLower() + "\":" + "\"" +
                                    (dt.Rows[i][j] != DBNull.Value ? string.Format("{0:yyyy-MM-dd}", dt.Rows[i][j]) : "") + "\",");
                            }
                            else
                            {
                                jsonStr.Append("\"" + dt.Columns[j].ColumnName.ToLower() + "\":" + "\"" + dt.Rows[i][j] + "\",");
                        ...
                        else if (j == dt.Columns.Count - 1)
                        {
                            ...
                            {
                                jsonStr.Append("\"" + dt.Columns[j].ColumnName.ToLower() + "\":" +
                                                  ReplaceSpecialCharacters(dt.Rows[i][j].ToString().ToLower()));
                            }
                            else if (dt.Columns[j].DataType == typeof(string))
                            {
                                jsonStr.Append("\"" + dt.Columns[j].ColumnName.ToLower() + "\":" + "\"" +
